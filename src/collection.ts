...
export class CollectionController<T = unknown> {
  ...
  private findNearestNavigable(previous: readonly CollectionItem<T>[], previousId: string | null): string | null { if (previousId === null) return null; const index = previous.findIndex((item) => item.id === previousId); for (let i = index + 1; i < previous.length; i++) if (this.isNavigable(previous[i].id)) return previous[i].id; for (let i = index - 1; i >= 0; i--) if (this.isNavigable(previous[i].id)) return previous[i].id; return this.items.find((item) => this.isNavigable(item.id))?.id ?? null; }
