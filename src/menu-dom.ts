...
export function bindMenu(root: HTMLElement): MenuBinding {
  ...
  const onClick = (event: MouseEvent): void => { const item = (event.target as HTMLElement).closest<HTMLElement>("[data-rui-menuitem]"); if (item?.parentElement === root) { controller.collection.setActive(item.id, "pointer", event); controller.selectActive(event); sync(); } };
