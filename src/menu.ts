...
export class MenuController<T = unknown> {
  ...
  handleKeyDown(event: KeyboardEvent): boolean { this.assertAlive(); if (event.key === "Escape") { this.close("escape"); return true; } if (!this.openState) return false; if (event.key === " ") { this.close("programmatic"); return true; } if (event.key === "ArrowDown") { this.collection.next(event); return true; } if (event.key === "ArrowUp") { this.collection.previous(event); return true; } if (event.key === "Home") { this.collection.first(event); return true; } if (event.key === "End") { this.collection.last(event); return true; } if (event.key === "Enter" && this.collection.getState().activeId) { this.emit("select"); return true; } return false; }
