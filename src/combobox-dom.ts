...
export function bindCombobox<T = string>(root: HTMLElement, options: ComboboxOptions<T> = {}): ComboboxBinding<T> {
  ...
  const sync = (): void => {
    ...
    const visibleIds = new Set(state.visibleItems.map((item) => item.id));
    for (const item of state.collection.items) if (item.element) { item.element.id = item.id; item.element.hidden = !visibleIds.has(item.id); item.element.setAttribute("role", "option"); item.element.setAttribute("aria-selected", String(state.collection.selectedIds.includes(item.id))); item.element.toggleAttribute("data-active", item.id === active); if (item.disabled) item.element.setAttribute("aria-disabled", "true"); else item.element.removeAttribute("aria-disabled"); }
    if (active) state.collection.items.find((item) => item.id === active)?.element?.scrollIntoView?.({ block: "nearest" });
  };
  ...
  const onReset = (): void => { queueMicrotask(() => { if (destroyed) return; (input as HTMLInputElement).value = initialInputValue; if (valueElement) valueElement.value = initialValue; controller.setInputValue(initialInputValue, "programmatic"); controller.collection.clearSelection(); if (initialValue) controller.select(initialValue); controller.setInputValue(initialInputValue, "programmatic"); sync(); }); };
