...
[![codecov](https://codecov.io/gh/vap-tech/rui-core/branch/main/graph/badge.svg)](https://codecov.io/gh/vap-tech/rui-core)
...
- DOM adapters — `bindListbox`, `bindCombobox`, `bindSelect`, `bindMenu`, `bindTabs`, `bindDialog`, `bindPopover`, Checkbox, Slider, Tooltip и Menu extensions.
- `RepUI.mount()` / `RepUI.unmount()` — безопасная инициализация DOM-фрагментов, включая HTMX/Django-сценарии; распознаются все перечисленные adapters.
...
Для Checkbox, Slider и Tooltip доступны headless-контроллеры и DOM adapters:

```ts
import { bindCheckbox, bindSlider, bindTooltip } from "@repui/core";

const checkbox = bindCheckbox(document.querySelector("[data-rui-checkbox]")!);
const slider = bindSlider(document.querySelector("[data-rui-slider]")!);
const tooltip = bindTooltip(document.querySelector("[data-rui-tooltip]")!);
```

`bindCheckboxGroup` поддерживает multiple selection и indeterminate state, `bindRangeSlider` — два thumb, а `bindMenubar` и `bindContextMenu` расширяют общую механику MenuController. Core не занимается CSS и позиционированием.
